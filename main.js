...
let HEIGHT = window.innerHeight; // Высота окна
let SIMULATION_WINDOW_HEIGHT = HEIGHT - PLOT_H;       // Высота окна с симуляцией
let SIMULATION_WINDOW_WIDTH = WIDTH - SETTINGS_WIDTH; // Ширина окна с симуляцией
let PLOT_W = SIMULATION_WINDOW_WIDTH;                 // Ширина графика энергии
const PLOT_X = SETTINGS_WIDTH;                        // Координата X графика энергии
let PLOT_Y = HEIGHT - PLOT_H;                         // Координата Y графика энергии
let ORIGIN_X = SIMULATION_WINDOW_WIDTH / 2 + SETTINGS_WIDTH; // Координата X точки подвеса
let ORIGIN_Y = SIMULATION_WINDOW_HEIGHT / 2;                 // Координата Y точки подвеса
let L_MAX = Math.min(SIMULATION_WINDOW_HEIGHT, SIMULATION_WINDOW_WIDTH) / 4;
L_MAX -= L_MAX % 10; // Максимальная длина плеча маятника
...
function recomputeLayout() {
    ...
    SIMULATION_WINDOW_WIDTH = WIDTH - SETTINGS_WIDTH;
    SIMULATION_WINDOW_HEIGHT = HEIGHT - PLOT_H;

    PLOT_W = SIMULATION_WINDOW_WIDTH;
    PLOT_Y = HEIGHT - PLOT_H;

    ORIGIN_X = SIMULATION_WINDOW_WIDTH / 2 + SETTINGS_WIDTH;
    ORIGIN_Y = SIMULATION_WINDOW_HEIGHT / 2;

    L_MAX = Math.min(SIMULATION_WINDOW_HEIGHT, SIMULATION_WINDOW_WIDTH) / 4;
    L_MAX -= L_MAX % 10;
}
